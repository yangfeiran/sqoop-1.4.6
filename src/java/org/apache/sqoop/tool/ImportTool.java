...
public class ImportTool extends com.cloudera.sqoop.tool.BaseSqoopTool {
    ...
    private boolean initIncrementalConstraints(SqoopOptions options,
        ...
        switch (incrementalMode) {
                ...
                try {
                    ...
                    if (isDateTimeColumn(checkColumnType)) {
                        ...
                        nextIncrementalValue = (nextVal == null) ? null
                                : manager.datetimeToQueryString(nextVal.toString(),
                                checkColumnType);
                        nextIncrementalValue = nextIncrementalValue.replaceAll("TO_DATE", "");
                        nextIncrementalValue = nextIncrementalValue.replaceAll(", 'YYYY-MM-DD HH24:MI:SS'", "");
                    } else {
            ...
            case DateLastModified:
                ...
                LOG.info("now time: " + s.format(nowTime.getTime()));
                nowTime.add(Calendar.MINUTE, -Integer.valueOf(options.getIncrementalOffset()));
                nextVal = s.format(nowTime.getTime());
                LOG.info("new now time: " + nextVal);
                if (null == nextVal) {
        ...
        if (isDateTimeColumn(checkColumnType) && null != prevEndpoint
            ...
            prevEndpoint = manager.datetimeToQueryString(prevEndpoint,
                    checkColumnType);
        }if(manager.isCharColumn(checkColumnType)&& null != prevEndpoint
                && !prevEndpoint.startsWith("\'") && !prevEndpoint.endsWith("\'")){
    ...
    protected RelatedOptions getIncrementalOptions() {
        ...
        incrementalOpts.addOption(OptionBuilder.withArgName("value")
                ...
                .create());
        incrementalOpts.addOption(OptionBuilder.withArgName("offset")
                .hasArg()
                .withDescription("The incremental offset")
                .withLongOpt(INCREMENT_OFFSET_ARG)
                .create());
    ...
    private void applyIncrementalOptions(CommandLine in, SqoopOptions out)
        ...
        if (in.hasOption(INCREMENT_TYPE_ARG)) {
            ...
            if (in.hasOption(INCREMENT_OFFSET_ARG)) {
                out.setIncrementalOffset(in.getOptionValue(INCREMENT_OFFSET_ARG));
            }else{
                out.setIncrementalOffset("5");
            }
