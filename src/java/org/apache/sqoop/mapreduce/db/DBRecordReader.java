...
public class DBRecordReader<T extends DBWritable> extends RecordReader<LongWritable, T> {
	...
	protected ResultSet executeQuery(String query) throws SQLException {
		...
		try {
			...
			query = query.replaceAll("TO_DATE", "");
			query = query.replaceAll(".0', 'YYYY-MM-DD HH24:MI:SS'", "'");
			query = query.replaceAll(", 'YYYY-MM-DD HH24:MI:SS'", "");
			query = query.replaceAll("\\.0", "'");
			query = query.replaceAll(">= ", ">= '");
			query = query.replaceAll("''","'");
			this.statement = connection.prepareStatement(query, ResultSet.TYPE_FORWARD_ONLY,
					ResultSet.CONCUR_READ_ONLY);
			fetchSize = dbConf.getFetchSize();
			if (fetchSize != null) {
				LOG.debug("Using fetchSize for next query: " + fetchSize);
				statement.setFetchSize(fetchSize);
			}
			LOG.info("Execution Query: " + query);
//			if(true)
//			throw new SQLException("Execution Query: " + query);
			return statement.executeQuery();
		}
	}
